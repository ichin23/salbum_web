...
const suggestedImageUrl = ref<string | null>(null);
const fetchingSuggestion = ref(false);

function acceptSuggestedImage() {
  image_url.value = suggestedImageUrl.value;
  suggestedImageUrl.value = null;
}

function dismissSuggestedImage() {
  suggestedImageUrl.value = null;
}
...
function msToMinSec(ms: number | null): { min: string; sec: string } {
...
onMounted(async () => {
  try {
    const data = await fetchReleaseDetails(albumId);
    const a = data.album;
    name.value = a.name;
    albumType.value = a.type ?? "";
    country.value = a.country ?? "";
    release_date.value = a.release_date ?? "";
    genres.value = a.genres ?? [];
    image_url.value = a.image_url ?? null;
    if (!a.image_url) {
    ...
    tracks.value = a.musics
      .slice()
      .sort((x, y) => x.position - y.position)
      .map((m, i) => {
        const { min, sec } = msToMinSec(m.length);
        return {
          name: m.name,
          length_min: min,
          length_sec: sec,
          position: i + 1,
          artistIds: m.artists.map((x) => x.id),
          artistNames: m.artists.map((x) => x.name),
          artistSearch: "",
          artistResults: [],
          searching: false,
          dropdownOpen: false,
    ...
    if (tracks.value.length === 0) tracks.value = [newTrack(1)];
  } catch (e) {
    loadError.value = e instanceof Error ? e.message : "Erro ao carregar álbum";
  } finally {
...
async function submit() {
  ...
  try {
    ...
    error.value = e instanceof Error ? e.message : "Erro ao salvar álbum.";
  } finally {
    saving.value = false;
  }
}
...
<template>
  <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6 pb-20">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-xl font-bold text-white">Editar álbum</h1>
        <p class="text-xs text-muted mt-0.5">Altere as informações do álbum</p>
      </div>
    </div>
    ...
    <!-- Load error -->
    <div v-else-if="loadError" class="text-red-400 text-sm text-center py-10">
      {{ loadError }}
    </div>
    ...
    <form v-else @submit.prevent="submit" class="space-y-6">
      ...
      <section class="card p-5 space-y-4">
        ...
        <div
          v-if="fetchingSuggestion"
          class="flex items-center gap-3 p-3 rounded-xl bg-[var(--color-surface-2)] border border-[var(--color-border)] text-xs text-muted"
        >
          <Loader2 class="w-4 h-4 animate-spin flex-shrink-0" />
          Buscando sugestão de capa no MusicBrainz...
        </div>
        <div
        ...
        >
          ...
          <div class="flex items-center gap-3">
            <img
              ...
              class="w-16 h-16 rounded-lg object-cover flex-shrink-0 border border-[var(--color-border)]"
              alt="Capa sugerida"
            />
            <div class="flex flex-col gap-1.5">
              ...
              <div class="flex gap-2">
                <button
                  type="button"
                  @click="acceptSuggestedImage"
                  class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-white text-xs font-semibold hover:bg-primary/90 transition-colors"
                >
                ...
                <button
                  type="button"
                  @click="dismissSuggestedImage"
                  class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[var(--color-surface-2)] border border-[var(--color-border)] text-muted text-xs hover:text-white transition-colors"
                >
                  <X class="w-3.5 h-3.5" />
                  Ignorar
                </button>
        ...
        <ImageUploadButton
          ...
          :on-upload="handleAlbumImageUpload"
          size="md"
          shape="rounded"
